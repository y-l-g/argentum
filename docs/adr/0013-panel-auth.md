...
- Password reset, registration, 2FA, multi-panel guards, and roles/RBAC remain open, each with a seam that does not need reopening: per-user session revocation, per-`Panel` `Auth` values, and `can_access_panel`. The `Policy` wire-or-remove decision is resolved — removed (GH #109, 2026-09-15; see ADR-0004), leaving `can_*` the one authorization vocabulary.
