...
`argentum-ui/src/components/` mixed re-exported Topcoat UI primitives (button, card, badge...) with owned composites (sidebar) in one flat folder, so a `topcoat-ui-registry` sync would conflict with Argentum edits. We split into `primitives/` — verbatim mirror of `topcoat-ui-registry/src/components/` synced via `cargo xtask sync-topcoat-ui` (header `// SYNC: topcoat-ui-registry@<version> sha256:<content-hash> — do not hand-edit`; drift is enforced by `xtask/tests/registry_sync.rs`) — and `composites/` — hand-written Argentum components (Sidebar, Page, CodeBlock) that compose primitives and Tokens. One crate is kept (`argentum-ui` re-exports both) to avoid crate proliferation; a two-crate split (`argentum-ui-primitives` + `argentum-ui`) was rejected.
...
**Status 2026-09-16:** the upstream `sidebar` landed (topcoat#419) and is synced into `primitives/sidebar.rs`; `composites/sidebar.rs` is deleted and `lib.rs` re-exports the primitive. `composites/` now holds the owned components only (page, code_block, error_state, theme, toast). The prediction below — "`composites/sidebar.rs` becomes a thin wrapper or is retired" — resolved as retired: the shell binds the primitive's runtime signals directly (ADR-0009 amendment).
...
**Status 2026-09-19 (GH #173):** `composites/code_block.rs` is deleted with its asset and re-export (zero callers; the `syntect` dependency it carried goes with it). `composites/` now holds the owned components only: page, error_state, theme, toast.
