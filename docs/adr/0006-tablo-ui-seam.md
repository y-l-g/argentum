...
+ `@source` for both the app's `src/**/*.rs` and `tablo-ui/src/**/*.rs`) and a 3-line `build.rs`
(`tablo_ui::tailwind_build()` / `topcoat::tailwind::BuildConfig`) plus `tailwind::stylesheet!()`
...
them. `tablo-ui` is the one `@source` an app's stylesheet must add, and that stays an ADR-visible
