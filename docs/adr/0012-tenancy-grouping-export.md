...
**Status 2026-09-22 (GH #204):** the `<40ms p50` Phase-2 budget named in the Consequences above is a **target, not a gate**. GH #103 closed without making it gate anything: the harness prints "reference only; UNGATED" (GH #171) and never PASS/FAILs on the number, and `benchmarks/results/` is gitignored, so a fresh checkout carries no measured result. README §10 and `CONTEXT.md` are marked accordingly; the showcase still demonstrates the list path the budget measures.

**Status 2026-09-22 (GH #223):** one clause of the #177 amendment above — "the tenancy scope is still `query`'s (ADR-0002)" — is superseded. The framework applies the tenant predicate itself, after whatever `query`/`export_query` returned, for a resource that declares `requires_tenant()`; `query` keeps the soft-delete and row-level half. The predicate is `Resource::tenant_scope`'s — derived from the model's `tenant_id` by default, declared by the resource when its rows inherit their tenant (the comments queue scopes through its parent post). See ADR-0002's 2026-09-22 amendment. The Decision section's hand-written `filter(tenant_id().eq(tid))` recipe and its `x-tenant-id` header fallback (already removed by GH #131) are both history.
