...
Date: 2026-09-21 — Status: accepted — Amended: 2026-09-25
...
**The tuple ceiling is eight, shared.** `IntoSchema`, `IntoColumns`, `IntoFilters` and
`IntoRelationColumns` are each generated by one `macro_rules!` invocation per arity 2..=8, so a view
with more than eight top-level blocks wraps the rest in a `Group`.
