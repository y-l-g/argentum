...
**Status 2026-09-22 (GH #204):** the `bulk.js` row above is stale in both directions. `data-bulk-submit` was retired by GH #184 — the disabled-until-selected submit became a confirmation dialog — and the row omits the three hooks that dialog added: `data-bulk-confirm-trigger`, `data-bulk-confirm-dialog`, `data-bulk-confirm-description`. The row as it stands today is `data-bulk-form` / `data-table-root` / `data-bulk-confirm-trigger` / `data-bulk-confirm-dialog` / `data-bulk-confirm-description` / `data-row-select` / `data-bulk-select-all` / `ids` transport, which is exactly what `xtask::ASSET_HOOKS` checks.

The size claim in the Context section is stale too, and by a wide margin. The 2026-09-18 figures were written against the then-eight assets — `bulk.js` at 2.8 KB and `selects.js` at 1.1 KB, with `code_block.js` still in the set, 15,066 bytes unminified in total. Re-measured today over the eight shipped assets (`selects.test.js` is the Node test, not shipped): **38,623 bytes unminified** and **13,766 bytes gzipped summed per asset** — each asset is its own response, so per-asset compression is what a browser actually receives; compressing all eight as one concatenated stream gives 11,066 bytes, which is not how they are served. `selects.js` alone is 17,860 bytes and `bulk.js` 7,348.
