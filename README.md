...
- `FileUpload` stores the client filename as the `String` path (bytes not persisted in v1, no `value` on `type=file`); forms containing one emit `enctype="multipart/form-data"`.
