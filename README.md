...
- Fields: `TextInput` (`required`/`email`/`unique`), `Select` (`options`/`options_with_labels`/`relationship`/`searchable`), `FileUpload`, `Repeater`. There is no `Toggle`, `exists`, or `regex` builder — relationship existence is checked app-side in the record handlers.
