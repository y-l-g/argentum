...
- **Streaming adopted** on topcoat PR #373: `suspense` regions with page-owned `signal`s (upstream's `live!`/`emit!` not used by Argentum yet). The resource list ships its shell + skeleton first and swaps the loaded grid in; a failed load renders the `error_boundary`-caught `ErrorState` inside the region instead of truncating the body (GH #79). Remaining: making the live-search `#[shard]` real — today the signal input coexists with the `?q=` toolbar while the shard still renders a stub (GH #74); `#[shard]` stays opt-in for isolated heavy widgets. Add dev lint for unmemoized deferred loads.
